...
## Prompt: Resumo Executivo de Review

Usar este prompt sempre que o utilizador fornecer uma transcrição de uma Sprint Review (Revisão da Sprint) e pedir um resumo, síntese ou consolidação. Não alterar a estrutura de output — é o formato canónico acordado para publicação na ferramenta de documentação interna.

**Goal:**
Gerar um resumo executivo, claro e alinhado ao negócio, a partir da transcrição da Sprint Review com o time de desenvolvimento e stakeholders.

**Context:**
Estas reuniões servem para demonstrar o valor entregue na sprint, coletar feedback do time de negócios e alinhar os próximos passos do produto. O objetivo é extrair o que foi aceito, os feedbacks recebidos e as mudanças de prioridade, evitando jargões técnicos excessivos.

**Source:**
Transcrição de reunião do Microsoft Teams (incluindo comentários informais, demonstrações de produto, dúvidas de stakeholders e possíveis redundâncias).

**Instructions:**
Analisa a transcrição da Sprint Review e produz um resumo estruturado e orientado ao negócio. Foca-te em identificar o que foi efetivamente entregue, as reações/feedbacks dos stakeholders e as definições para o futuro do produto.
...
2. **Entregas e Funcionalidades Demonstradas**
   - Lista das principais features, melhorias ou correções apresentadas.
   - Indicação clara se a entrega foi Aceita/Aprovada ou se necessita de ajustes.

3. **Feedback dos Stakeholders / Time de Negócios**
   - Pontos Fortes: (O que o negócio elogiou ou validou como alto valor)
   - Dúvidas/Preocupações: (Dores trazidas pelo negócio, pontos de atrito ou dúvidas operacionais levantadas)

4. **Novos Requisitos e Ajustes (Mudanças no Backlog)**
   - Solicitações de alteração, novos insights ou melhorias que surgiram durante a demonstração.
   - Diferenciar o que é impeditivo (deve ser feito logo) do que é melhoria futura (ideias para o backlog).

5. **Próximos Passos e Alinhamento do Roadmap**
   - Qual é o foco principal da próxima sprint acordado na reunião.

   - Datas importantes, alinhamentos pendentes ou decisões estratégicas que precisam ser tomadas.

**Additional Guidelines:**

- Linguagem de Negócio: Traduzir termos técnicos complexos para linguagem de produto/valor de negócio (ex: em vez de "refatoração da API x", usar "melhoria de performance no sistema de paginação").
- Evitar repetir conteúdo redundante ou conversas informais da demonstração.
- Priorizar clareza e linguagem executiva para que stakeholders que não participaram consigam ler e entender o status do produto.
- Se houver divergência sobre o escopo ou aceitação de uma entrega, sintetizar de forma neutra destacando os pontos de vista.
